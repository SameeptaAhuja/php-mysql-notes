...
if (isset($_POST["submit"])) 
{
    ...
    {
        # code to validate the email if pre-exist or not
        $fh = fopen("members.txt","r");
        while(list($c_sn,$c_name1,$c_name2,$c_email,$c_gender)= fgetcsv($fh,1024,','))
        {
            
            if ($c_email == $email)
            {
                echo '<script>alert("User aready registered")</script>';
                $error .= '<p></p>';
                break;
            } 
            else
    ...
    if ($error == '') {
        $file_open = fopen("members.txt", "a");
        $no_rows = count(file("members.txt"));
        if ($no_rows > 1) {
            $no_rows = ($no_rows - 1) + 1;
        }
        $form_data = array(
            ...
            <form action="" method="post">
                <div class="container">
                    ...
                    <div id="Email_ID_">
                        ...
                        <input type="email" name="email" value="<?php echo $email; ?>" required="email">
                    </div>
                    <div id="Gender_">
                        <span>Gender :</span>
                    </div>
